...
function getMarkit(req, res){
	...
	var Symbol = message.body.replace(bot.mention_text , "");
	

	var path = 'http://dev.markitondemand.com/MODApis/Api/v2/Quote/jsonp?symbol='+Symbol.trim()+'&callback='+Symbol.trim() ;
	...
	request.get(
	    ...
	    { 
	    	...
	    	console.log(error)
	        if (!error && response.statusCode == 200) {
	           res.status(200).send({'body': body, 'is_reply':true, 'open_sidebar':false});
	        }else{
	        	console.log("error")
	        	res.status(500).send({'body': "server error, conversation not updated", 'status': "error"});
	        }
